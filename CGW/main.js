...
let sphereSurface;
let context;
let audio = null, source, biquadFilter, panner;
...
function draw() {
    ...
    const spatialAudio = vecFromAngles(alpha, beta, gamma)
    gl.uniformMatrix4fv(shProgram.iModelViewProjectionMatrix, false, m4.translation(...spatialAudio));
    sphereSurface.Draw()
    if (panner) {
        panner.setPosition(...spatialAudio)
    }
    gl.clear(gl.DEPTH_BUFFER_BIT)
    ...
    // if (gyroStart) {
    modelViewProjection = m4.multiply(stereoCamera.projection, m4.multiply(stereoCamera.modelView, m4.multiply(matAccum1, fusionMat)));
    // }
    gl.uniformMatrix4fv(shProgram.iModelViewProjectionMatrix, false, modelViewProjection);
...
function initGL() {
    ...
    sphereSurface = new Model()
    sphereSurface.BufferData(
        createSphereSurface(),
        createSphereSurface(),
        createSphereSurface(),
    )
...
function createSphereSurface(r = 1) {
    let vertexList = [];
    let lon = -Math.PI;
    let lat = -Math.PI * 0.5;
    const STEP = 0.1;
...
function setupAudio() {
    ...
    audio.addEventListener('play', () => {
        if (!context) {
            ...
            biquadFilter.frequency.value = 808;
...
function initAudio() {
    setupAudio();
    let radioButton = document.getElementById('lowshelf');
...
function vecFromAngles(a, b, g) {
    // Convert angles to radians
    const alphaRad = deg2rad(a)
    const betaRad = deg2rad(b)
    const gammaRad = deg2rad(g)
